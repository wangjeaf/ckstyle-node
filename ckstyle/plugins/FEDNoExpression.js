...
module.exports = global.FEDNoExpression = new Class(RuleChecker, function () {
		...
		this.check = function (self, rule, config) {
			...
			//console.log(name,rule.value,rule.roughValue,rule.strippedValue)//解不出expression
			...
			if (replaced.indexOf('Expressions') != -1 || replaced.indexOf('this.style.' + name + '=') != -1 || replaced.indexOf('this.runtimeStyle.' + name + '=') != -1) {
		...
		this.__doc__ = {
			"summary" : "不要使用非一次性表达式",
			"desc" : "IE下，非一次性expression对性能有很大的影响，或许一次鼠标移动，将触发<strong>成千上万次</strong>的expression表达式的执行，因此，为了浏览器的更新换代，应该杜绝使用非一次性表达式。<br>本工具针对一次性表达式的检查，将判断expression中是否有如下两个内容：<br>1. <code>Expressions</code><br>2. <code>this.style.attrName = </code>"
		}
	})